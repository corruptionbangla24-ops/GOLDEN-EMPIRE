...
// 🏛️ ৫-রিলস ওরিজিনাল গোল্ডেন এম্পায়ার ইনকা ক্যাসিনো র্যান্ডমাইজেশন প্রতীক তালিকা ভাই ভাই
const goldenEmpirePool = ["INCA_CHIEF", "GOLDEN_MASK", "SUN_DISK", "GOLD_MEDAL", "SACRED_TOTEM", "LUCKY_RING", "ANCIENT_COIN"];

// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে
app.get('/api/empire-balance', async (req, res) => {
...
// 🛫 ২. গোল্ডেন এম্পায়ার ৫-রিল কোর স্লট স্পিন রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/empire-spin', async (req, res) => {
    const { userId, amount, wallet } = req.body;
    const targetWallet = wallet || "main";
    const reqAmount = parseFloat(amount) || 50;

    // 🔒 [বেট সিকিউরিটি ফিল্টার]: বাজি ১ টাকার কম বা ২০০০০ টাকার বেশি হলে ব্যাকএন্ড ডিরেক্টブロック ভাই ভাই!
    ...
    try {
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.empire_target) ? balResponse.data.empire_target : null;

        let spinResults, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP ও ৫-রিলস গোল্ডেন এম্পায়ার গাণিতিক লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            ...
            spinResults = [];
            for (let r = 0; r < 5; r++) {
                spinResults.push(goldenEmpirePool[Math.floor(Math.random() * goldenEmpirePool.length)]);
            }
            ...
            if (maxSameSymbolsCount >= 3) {
                finalStatus = "win";
                // 🚀 [ডাইনামিক কম্বো মাল্টিপ্লায়ার ওডস]: ৩টি মিললে ৬.৫০ গুণ, ৪টি মিললে ৪৫ গুণ, ৫টি মিললে ৪৫০ গুণের মেগা এম্পায়ার জ্যাকপট!
                if (maxSameSymbolsCount === 3) winMultiplier = 6.50;
                else if (maxSameSymbolsCount === 4) winMultiplier = 45.00;
                else winMultiplier = 450.00; 
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            }
            ...
            if (adminTriggeredPrize) {
                ...
                if (adminTriggeredPrize === "force_jackpot" && winMultiplier === 450.00) isLoopActive = false;
            } else {
                // ৪৫০ গুণের মেগা জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে কড়া সুরক্ষায় টাইট ০.০১% এ লক ভাই ভাই
                if (winMultiplier === 450.00 && Math.random() > 0.001) continue;
        ...
        console.error("Golden Empire Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Royal Golden Empire 5-Reel Slot Engine!"); });

// গোল্ডেন এম্পায়ার গেম নিজস্ব কাস্টম ৬৩০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 6300; 
server.listen(PORT, () => { console.log(`🎡 Royal Golden Empire Engine Running on port ${PORT}`); });
